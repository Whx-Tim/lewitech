...
class WechatOAuthAuthenticate
{
    ...
    {
        $type = null;
        if (in_array($scopes, $this->special_string)) {
            $type = $scopes;
            $scopes = null;
        }
        $isNewSession = false;
        ...
        if (!session('wechat.oauth_user') || $this->needReauth($scopes)) {
            if ($request->has('code')) {
                ...
                //非easy wechat定义内容
                Event::fire(new WechatUserLogin(session('wechat.oauth_user')));
                //
        ...
        Event::fire(new WeChatUserAuthorized(session('wechat.oauth_user'), $isNewSession));
        //非easy wechat定义内容
        Event::fire(new WechatUserLogin(session('wechat.oauth_user'), $type));
        //
